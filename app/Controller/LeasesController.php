...
class LeasesController extends AppController {
        ...
        public function printContract($id = null){
        	...
        	$data = array(
        			...
        			"location" => $con['Location']['name'], "address" => $con['Apartment']['address'],
        			"amount" => number_format($con['Lease']['amount'], 0, '.', '.'),
        			"end_d_day" => $end_d[2],"end_d_month" => $end_d[1], "end_d_year" => $end_d[0]
        	);
        ...
        public function custoValidation($lease){
        	 
        	$message=null;
        	$this->log($lease);
        	if ($lease['Lease']['location_id'] < 1){
        		$message = 'Ubicación: Debe ingresar una ubicació a rentar.';
        	}else if ($lease['Lease']['apartment_id'] < 1){
        		$message = 'Apartamento: Debe ingresar un apartamento/local a rentar.';
        	}else if($lease['Lease']['amount'] < 1){
        ...
        public function error(){
            ...
            //$this->redirect(array('controller' => 'Leases', 'action' => 'error'));
            $this->layout = 'home';
        }
