...
class PaymentsController extends AppController{
    ...
    public function add($leaseId = null){
        ...
        if ($this->request->is('post')){         	
             ...
             if($this->Payment->save($this->request->data)){
                 ...
                 $this->redirect(array('action' => 'printReceipt', $this->Payment->id ));
             }else{
    ...
    public function printReceipt($id = null){
    	if ($id !=null){
    		...
    		$this->loadModel('Apartment');
    		$this->Apartment->id = $payment['Lease']['apartment_id'];
    		$apartment = $this->Apartment->read();    		
    		$payment['Apartment'] = $apartment['Apartment'];
    		
    		$this->set('payment',$payment);
    		
    	}else{
    		$this->Session->setFlash("<div class = 'info'>Debe seleccionar un pago para imprimir.</div>");
    		$this->redirect(array('action' => 'view'));
    	}    	
    	$this->layout = 'printLay-Receipt';
    }
    ...
    public function remove($id){
    	...
    	if ($id !=null){
    		...
    		$this->Payment->id = $id;
    		$currpayment = $this->Payment->read();
    		
    		$lastpayment = $this->Payment->find('first', 
    				array('conditions' => array('lease_id' => $currpayment['Payment']['lease_id']),
    				'order' => array('to_date' => 'DESC')));     				
    		
    		//validacion de pagos mas nuevos
    		if (date($currpayment['Payment']['to_date']) < date($lastpayment['Payment']['to_date'])){
    			$this->Session->setFlash("<div class = 'err'>Error :Debe eliminar el &uacute;ltimo pago primeramente.</div>");
    			$this->redirect(array('action' => 'view'));
    			return;
    		}    		
    		
    		$this->Payment->id = $currpayment['Payment']['id'];
